...
namespace IMS.Infrastructure.Repositories
{
    public class ItemRepository : IItemRepository
    {
        ...
        {
            Maintenance? lastMaintenance = _dbContext.maintenances.Where(m => m.ItemId == id && m.Status != "Canceled" && m.IsActive).OrderByDescending(m => m.EndDate).FirstOrDefault();
            return _dbContext.items.Where(i => i.ItemId == id && i.IsActive).Select(i => new ItemDetailedDTO
            {
                itemId = i.ItemId,
        ...
        {
            return _dbContext.items.Where(i => i.IsActive && i.EquipmentId == equipmentId).Select(i => new ItemDTO
            {
                itemId = i.ItemId,
                imageUrl = i.Equipment.ImageURL,
                equipmentId = i.EquipmentId,
        ...
        {
            Item? existingItem = _dbContext.items.Where(i => i.EquipmentId == equipmentId && i.SerialNumber == serialNumber && i.IsActive).FirstOrDefault();
        ...
        {
            // delete item
            _dbContext.items.Where(i => i.ItemId == id).ExecuteUpdate(i => i.SetProperty(i => i.IsActive, false));
            // delete item reservations of the item
            _dbContext.itemReservations.Where(ir => ir.ItemId == id).ExecuteUpdate(ir => ir.SetProperty(ir => ir.IsActive, false));
            // delete maintenance logs of the item
            _dbContext.maintenances.Where(m => m.ItemId == id).ExecuteUpdate(m => m.SetProperty(m => m.IsActive, false));
            _dbContext.SaveChanges();
            return true;
        }
