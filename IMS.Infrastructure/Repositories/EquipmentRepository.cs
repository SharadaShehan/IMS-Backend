...
namespace IMS.Infrastructure.Repositories
{
    public class EquipmentRepository : IEquipmentRepository
    {
        ...
        {
            return _dbContext.equipments.Where(e => e.EquipmentId == id && e.IsActive).FirstOrDefault();
        }
        ...
        {
            return _dbContext.equipments.Where(e => e.IsActive && e.LabId == labId).Select(e => new EquipmentDTO
            {
                equipmentId = e.EquipmentId,
                name = e.Name,
                model = e.Model,
        ...
        {
            Equipment? existingEquipment = _dbContext.equipments.Where(e => e.Name == name && e.Model == model && e.LabId == labId && e.IsActive).FirstOrDefault();
            return existingEquipment != null;
        }

        public EquipmentDetailedDTO? CreateNewEquipment(CreateEquipmentDTO createEquipmentDTO, Lab lab)
        {
        ...
        public EquipmentDetailedDTO? UpdateEquipment(Equipment equipment, UpdateEquipmentDTO updateEquipmentDTO)
        {
            if (equipment == null) return null;
            if (updateEquipmentDTO.name != null) equipment.Name = updateEquipmentDTO.name;
            if (updateEquipmentDTO.model != null) equipment.Model = updateEquipmentDTO.model;
            if (updateEquipmentDTO.imageURL != null) equipment.ImageURL = updateEquipmentDTO.imageURL;
            if (updateEquipmentDTO.specification != null) equipment.Specification = updateEquipmentDTO.specification;
            if (updateEquipmentDTO.maintenanceIntervalDays != null) equipment.MaintenanceIntervalDays = (int)updateEquipmentDTO.maintenanceIntervalDays;
        ...
        {
            // delete equipment
            _dbContext.equipments.Where(e => e.EquipmentId == id).ExecuteUpdate(e => e.SetProperty(e => e.IsActive, false));
            // delete items of the equipment
            _dbContext.items.Where(i => i.EquipmentId == id).ExecuteUpdate(i => i.SetProperty(i => i.IsActive, false));
            // delete item reservations of the equipment
            _dbContext.itemReservations.Where(ir => ir.EquipmentId == id).ExecuteUpdate(ir => ir.SetProperty(ir => ir.IsActive, false));
            // delete maintenance logs of the equipment
            _dbContext.maintenances.Where(m => m.Item.EquipmentId == id).ExecuteUpdate(m => m.SetProperty(m => m.IsActive, false));
            _dbContext.SaveChanges();
            return true;
        }
