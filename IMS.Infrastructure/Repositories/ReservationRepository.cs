...
namespace IMS.Infrastructure.Repositories
{
    public class ReservationRepository : IReservationRepository
    {
        ...
        {
            return _dbContext.itemReservations.Where(rsv => rsv.IsActive && (rsv.Status == "Pending" || rsv.Status == "Reserved" || rsv.Status == "Borrowed")).Select(rsv => new ItemReservationDTO
            {
                reservationId = rsv.ItemReservationId,
                equipmentId = rsv.EquipmentId,
                itemName = rsv.Equipment.Name,
                ...
                cancelledAt = rsv.CancelledAt,
                status = rsv.Status
            }).OrderByDescending(rsv => rsv.createdAt).ToList();
        }

        public List<ItemReservationDTO> GetAllRequestedReservationDTOs()
        {
            return _dbContext.itemReservations.Where(rsv => rsv.IsActive && rsv.Status == "Pending").Select(rsv => new ItemReservationDTO
            {
                reservationId = rsv.ItemReservationId,
                equipmentId = rsv.EquipmentId,
                itemName = rsv.Equipment.Name,
        ...
        {
            return _dbContext.itemReservations.Where(rsv => rsv.IsActive && rsv.Status == "Reserved").Select(rsv => new ItemReservationDTO
            {
                reservationId = rsv.ItemReservationId,
                equipmentId = rsv.EquipmentId,
                itemName = rsv.Equipment.Name,
                ...
                cancelledAt = rsv.CancelledAt,
                status = rsv.Status
            }).OrderByDescending(rsv => rsv.respondedAt).ToList();
        }

        public List<ItemReservationDTO> GetAllBorrowedReservationDTOs()
        {
            return _dbContext.itemReservations.Where(rsv => rsv.IsActive && rsv.Status == "Borrowed").Select(rsv => new ItemReservationDTO
            {
                reservationId = rsv.ItemReservationId,
                equipmentId = rsv.EquipmentId,
                itemName = rsv.Equipment.Name,
                ...
                cancelledAt = rsv.CancelledAt,
                status = rsv.Status
            }).OrderByDescending(rsv => rsv.borrowedAt).ToList();
        }

        public List<ItemReservationDTO> GetAllBorrowedReservationDTOsByStudent(int studentId)
        {
            return _dbContext.itemReservations.Where(rsv => rsv.IsActive && rsv.ReservedUserId == studentId && rsv.Status == "Borrowed").Select(rsv => new ItemReservationDTO
            {
                reservationId = rsv.ItemReservationId,
                equipmentId = rsv.EquipmentId,
                itemName = rsv.Equipment.Name,
        ...
        {
            return _dbContext.itemReservations.Where(rsv => rsv.IsActive && rsv.ReservedUserId == studentId && (rsv.Status == "Reserved" || rsv.Status == "Rejected" || rsv.Status == "Pending")).Select(rsv => new ItemReservationDTO
            {
                reservationId = rsv.ItemReservationId,
                equipmentId = rsv.EquipmentId,
                itemName = rsv.Equipment.Name,
        ...
        public ItemReservationDetailedDTO? RequestEquipmentReservation(Equipment equipment, User student, RequestEquipmentDTO requestEquipmentDTO)
        {
        ...
        public ItemReservationDetailedDTO? RejectEquipmentReservation(ItemReservation reservation, User clerk, RespondReservationDTO respondReservationDTO)
        {
            reservation.RespondedClerkId = clerk.UserId;
            reservation.RespondedClerk = clerk;
        ...
        public ItemReservationDetailedDTO? BorrowReservedItem(ItemReservation reservation, User clerk)
        {
            if (reservation.Item == null) return null;
